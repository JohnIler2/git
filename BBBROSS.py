...
def BayraceScenario():
    ...
    global TimeSpent
    global Inventory
    global BayraceScenario

   
    #Setup the default map
    BayraceScenario=Maps.Map(BayMap,3,3,"The bay water is choppy today.")
    CurrentTile=BayraceScenario.GetCurrentTile()
    
    print("\nA You are looking forward to a day in the bay hunting alligators to make alligator jerky.\n")
    print("You also have a reputation as the fastest cajun in the bayou and have heard that Swamp Racer is looking for you to race.")
    if Inventory.count("rifle") >0 and Inventory.count("lucky hat")>0:
        Inventory.append("rifle")
        Inventory.append("lucky hat")
        
    print ("You gather up your lucky hat and rifle from under the boat seat")
    while CheckTimer():
...
###### END EVENT CODE!!!!
        ...
        if UserSelection=="X":
            ...
            if CurrentTile.GetItem()!="":
                ...
                print("You find {}.".format(UserSelection,CurrentTile.GetItem()))
            ...
            #ToDo?  Make alligator hunting so it isn't a waste of time????
            UserSelection=int(raw_input("How many minutes would you like to stay and hunt alligator? "))
            if UserSelection==0:
                print("You decide to skip alligator hunting for now.")
            else:
...
RiverFluffTiles.append(Tiles.Tile("choppy, murky water",0,"",""))
...
BayTiles.append(Tiles.Tile("You see Swamp Racer appear",5,"turbo prop, moon shine, and lucky hat increases chance of winning","race"))
BayTiles.append(Tiles.Tile("You board your boat at the marina",5,"discover your rifle and lucky hat on-board","marina"))
BayTiles.append(Tiles.Tile("You see something shimmering in the water",5,"pick up turbo prop","prop"))
BayTiles.append(Tiles.Tile("You see a floating trot line in the water",5,"collect moon shine bottle attached to trot line","moon shine"))
...
#These are the filler scenario tiles.  Don't put items here.  You can do it separately when the GenerateMap() code is called  You can put events here.
BayFluffTiles.append(Tiles.Tile("clear, open water",5,"",""))
BayFluffTiles.append(Tiles.Tile("choppy water",5,"",""))
